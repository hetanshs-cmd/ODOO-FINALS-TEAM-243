...
export const paymentsService = {
  async recordPayment(
    ...
    if (invoice.status === 'PAID' || invoice.status === 'VOID') {
      throw Errors.businessRuleViolation(`Cannot record a payment on an invoice in status ${invoice.status}`);
    }
    ...
    return withTransaction(async (client) => {
      ...
      const totalPaid = roundMoney(await paymentsRepository.sumSuccessfulForInvoice(client, invoiceId));
